...
[Proof templates](proof-templates.md) are used to create proof requests. When a proof request is created, you will receive a URL which needs to be delivered to a holder's wallet application either as a link (in the "response\_url" field) or displayed in a QR code for the wallet application to scan (conveniently pre-generated for you in the "qr" field). The holder's wallet will then generate a proof response and deliver it to the Truvera API which will check it for cryptographic correctness. Your [integration paradigm](../getting-started.md#integration-paradigms) will determine how you receive the verification response along with the verified attributes: either by polling or via a webhook.

To get a response by polling, check [the proof-request details](proof-requests.md#get-the-details-of-an-existing-proof-request) for when the "verified" attribute equals "true". At that point, the "presentation" field will contain the verified attributes as a JSON list.
