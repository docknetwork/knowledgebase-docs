...
The [Truvera Cloud Wallet](wallet-sdk/cloud-wallet/) is a hosted service for storing credential data so that it can be accessed outside of a specific mobile application. This can be used for credential recovery onto other device or credential usage in a web browser flow. The data is encrypted before storage using the Wallet SDK and a key generated by the user, so the data cannot be accessed by anyone else.
